...
public class CategoryRepositoryWithBagRelationshipsImpl implements CategoryRepositoryWithBagRelationships {
    ...
    public Optional<Category> fetchBagRelationships(Optional<Category> category) {
        return category.map(this::fetchContacts);
    }
    ...
    public List<Category> fetchBagRelationships(List<Category> categories) {
        return Optional.of(categories).map(this::fetchContacts).orElse(Collections.emptyList());
    }

    Category fetchContacts(Category result) {
        return entityManager
            .createQuery(
                "select category from Category category left join fetch category.contacts where category is :category",
                Category.class
            )
    ...
    List<Category> fetchContacts(List<Category> categories) {
        ...
        List<Category> result = entityManager
            .createQuery(
                "select distinct category from Category category left join fetch category.contacts where category in :categories",
                Category.class
            )
